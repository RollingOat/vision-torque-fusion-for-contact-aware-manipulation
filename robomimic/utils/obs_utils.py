...
def normalize_dict(dict, normalization_stats):
    ...
    for m in dict:
        ...
        shape_len_diff = len(dict[m].shape) - o_num_dims
        assert shape_len_diff >= 0, f"shape length mismatch in @normalize_dict: dict[{m}].shape={dict[m].shape}, offset.shape={offset.shape}"
        assert dict[m].shape[-o_num_dims:] == offset.shape, f"shape mismatch in @normalize_obs: dict[{m}].shape={dict[m].shape}, offset.shape={offset.shape}, last {o_num_dims} dims={dict[m].shape[-o_num_dims:]}"
...
class PointCloudModality(Modality):
    ...
    Supports both XYZ (3-channel) and XYZRGB (6-channel) point clouds.
    A single sample has shape (N, C) where N is the number of points and C is
    either 3 (XYZ) or 6 (XYZRGB).  No channel reordering is applied -- the data
    is passed through as float32, ready to be consumed by PointCloudCore.

    Register observation keys as this modality in the config::

        observation.modalities.obs.point_cloud = ["point_cloud"]
    """
    name = "point_cloud"
    ...
    def _default_obs_processor(cls, obs):
        """
        Convert point cloud to float32.  Shape (N, C) is preserved as-is.
        """
    ...
    def _default_obs_unprocessor(cls, obs):
        """
        Pass point cloud through unchanged.
        """
        return obs
