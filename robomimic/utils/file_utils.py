...
def get_shape_metadata_from_dataset(dataset_config, action_keys, all_obs_keys=None, obs_key_current_dim=None, verbose=False):
    """
    ...
    for k in sorted(all_obs_keys):
        if k == LangUtils.LANG_EMB_OBS_KEY:
            ...
            # If this key matches an obs_key_current_dim pattern, reduce its
            # shape to the last N elements (current measurement only).
            if obs_key_current_dim:
                for pattern, n_dims in obs_key_current_dim.items():
                    if pattern in k and len(initial_shape) == 1 and initial_shape[0] > n_dims:
                        initial_shape = (n_dims,)
                        break
        if verbose:
