...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        Args:
            ...
            obs_key_current_dim (dict or None): mapping from obs key name substring to number of
                dimensions to keep (last N elements). For obs keys that store a history of
                measurements concatenated along the last axis, this selects only the most-recent
                measurement. Example: {"torque": 7, "force": 6}. Defaults to None (no slicing).
        """
        ...
        self.obs_key_current_dim = obs_key_current_dim or {}
    ...
    def _apply_obs_key_current_dim(self, obs_key, data):
        """
        If @obs_key matches a pattern in self.obs_key_current_dim, slice @data along
        its last axis to keep only the last N elements (the current measurement).
        Returns data unchanged if no pattern matches.
        """
        for pattern, n_dims in self.obs_key_current_dim.items():
            if pattern in obs_key and data.shape[-1] > n_dims:
                return data[..., -n_dims:]
        return data
    ...
    def normalize_obs(self):
        ...
        obs_traj = {k: self._apply_obs_key_current_dim(k, v) for k, v in obs_traj.items()}
    ...
    def get_sequence_from_demo(self, demo_id, index_in_demo, keys, num_frames_to_stack=0, seq_length=1):
        ...
        for k in keys:
            ...
            # For obs/next_obs keys: apply current-dim slicing if configured.
            # Key format is "<prefix>/<obs_key_name>", e.g. "obs/robot0_joint_ext_torque".
            if '/' in k and self.obs_key_current_dim:
                obs_key_name = k.split('/', 1)[1]
                seq[k] = self._apply_obs_key_current_dim(obs_key_name, seq[k])
