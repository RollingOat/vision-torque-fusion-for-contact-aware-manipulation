#!/usr/bin/env python3
"""
Extract agentview_image frames from a specified demo in an HDF5 dataset.
Every 10th frame is saved (1 frame kept, 9 skipped).

Usage:
    python extract_agentview_images.py \
        --hdf5 /media/jiuzl/PortableSSD/egg_boiler/dual_camera/synced_image_data.hdf5 \
        --demo demo_0 \
...
def extract_agentview_images(hdf5_path, demo_name, output_dir, skip=9):
    """
    Extract agentview_image frames from a demo, keeping 1 out of every (skip+1) frames.
    ...
    with h5py.File(hdf5_path, "r") as f:
        key = f"data/{demo_name}/obs/agentview_image"
        if key not in f:
...
def main():
    parser = argparse.ArgumentParser(description="Extract agentview images from HDF5 demo.")
