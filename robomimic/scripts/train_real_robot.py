...
def train(config, device, resume=False):
    ...
    for dataset_cfg in config.train.data:
        ...
        try:
            ...
            print(f"\nWARNING: unable to load environment metadata from dataset - using dummy metadata. Error: {e}\n")
            env_meta = dict(env_name="RealRobot", type=999, env_kwargs={})
        ...
        shape_meta = FileUtils.get_shape_metadata_from_dataset(
            ...
            obs_key_current_dim=config.train.obs_key_current_dim,
            verbose=True
        )
    ...
    # If validation is enabled, ensure filter keys are set
    if config.experiment.validate:
      with config.values_unlocked():
          if config.train.hdf5_filter_key is None:
              print("Validation enabled but hdf5_filter_key not set in config. Defaulting to 'train'.")
              config.train.hdf5_filter_key = "train"
          if config.train.hdf5_validation_filter_key is None:
              print("Validation enabled but hdf5_validation_filter_key not set in config. Defaulting to 'valid'.")
    ...
    for epoch in range(start_epoch, config.train.num_epochs + 1):
        ...
        # (Rollouts functionality removed for real robot training)
        video_paths = None
...
def main(args):
    ...
    # Force disable rollouts for real robot training
    with config.values_unlocked():
