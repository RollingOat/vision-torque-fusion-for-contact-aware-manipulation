"""
Config for Diffusion Policy with Auxiliary Task algorithm.
"""

from robomimic.config.base_config import BaseConfig

class DiffusionPolicyAuxConfig(BaseConfig):
    ALGO_NAME = "diffusion_policy_aux"

    def train_config(self):
        """
        Setting up training parameters for Diffusion Policy with Auxiliary Task.

        - ENABLE next_obs loading from hdf5 to get force/torque observations
        - set compatible data loading parameters
        """
        super(DiffusionPolicyAuxConfig, self).train_config()
        
        # ENABLE next_obs loading from hdf5 for force/torque auxiliary task
    ...
    def algo_config(self):
        ...
        self.algo.auxiliary.force_torque_loss_weight = 1.0  # weight for force/torque auxiliary task loss
