...
class BaseConfig(Config):
    ...
    def train_config(self):
        ...
        # Mapping from obs key name substring to number of dimensions to keep (last N elements).
        # For obs keys that store a history of measurements (e.g. torque/force with multiple
        # timesteps concatenated), set this to use only the most-recent measurement.
        # Example: {"torque": 7, "force": 6}
        # An empty dict (default) disables slicing for all keys.
        self.train.obs_key_current_dim = {}
    ...
    def observation_config(self):
        ...
        self.observation.modalities.obs.point_cloud = []      # specify point cloud observations for agent (XYZ or XYZRGB)
        ...
        self.observation.modalities.goal.point_cloud = []     # specify point cloud goal observations
        ...
        # =============== Point Cloud default encoder (PointNet-based) ===============
        # Uses PointCloudCore, which implements a PointNet encoder that supports both
        # XYZ (3-channel) and XYZRGB (6-channel) inputs selectable via `input_type`.
        self.observation.encoder.point_cloud = deepcopy(self.observation.encoder.low_dim)
        self.observation.encoder.point_cloud.core_class = "PointCloudCore"      # PointNet encoder; see models/obs_core.py
        self.observation.encoder.point_cloud.core_kwargs = Config()             # Kwargs forwarded to PointCloudCore.__init__
        self.observation.encoder.point_cloud.core_kwargs.do_not_lock_keys()
