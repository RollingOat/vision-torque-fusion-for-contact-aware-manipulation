...
class DiffusionPolicyTransformer(PolicyAlgo):
    """
    Diffusion Policy with a transformer as the noise prediction network.

    Unlike the UNet variant, observation features are NOT flattened into a
    single global conditioning vector.  Instead they are passed as a sequence
    of tokens (B, To, obs_dim) to the TransformerForDiffusion model, which
    attends over [timestep | obs | noisy_actions] jointly.
    """
    ...
    def _create_networks(self):
        ...
        # replace BatchNorm with GroupNorm so EMA works correctly
        obs_encoder = replace_bn_with_gn(obs_encoder)

        obs_dim = obs_encoder.output_shape()[0]

        noise_pred_net = DPNets.TransformerForDiffusion(
            input_dim=self.ac_dim,
            output_dim=self.ac_dim,
            horizon=self.algo_config.horizon.prediction_horizon,
            obs_dim=obs_dim,
            obs_horizon=self.algo_config.horizon.observation_horizon,
            n_layer=self.algo_config.transformer.num_layers,
            n_head=self.algo_config.transformer.num_heads,
            n_emb=self.algo_config.transformer.embed_dim,
            p_drop_emb=self.algo_config.transformer.emb_dropout,
            p_drop_attn=self.algo_config.transformer.attn_dropout,
        )
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            ...
            obs_features = TensorUtils.time_distributed(inputs, self.nets["policy"]["obs_encoder"], inputs_as_kwargs=True)
            assert obs_features.ndim == 3  # (B, To, obs_dim)

            # keep obs as a token sequence - do NOT flatten
            obs_cond = obs_features  # (B, To, obs_dim)
            ...
            # predict the noise residual
            noise_pred = self.nets["policy"]["noise_pred_net"](
                noisy_actions, timesteps, obs_cond=obs_cond)
    ...
    def _get_action_trajectory(self, obs_dict, goal_dict=None):
        ...
        # keep as token sequence - do NOT flatten
        obs_cond = obs_features  # (B, To, obs_dim)
