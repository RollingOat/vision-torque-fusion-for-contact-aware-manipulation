...
class RolloutPolicy(object):
    ...
    def __init__(self, policy, obs_normalization_stats=None, action_normalization_stats=None):
        ...
        # Make stats accessible from inside the algo (e.g. for unnormalizing
        # observations during inference without going through RolloutPolicy).
        self.policy.obs_normalization_stats = obs_normalization_stats
    ...
    def _prepare_observation(self, ob, batched_ob=False, postprocess_visual_obs=True):
        ...
        # postprocess visual observations BEFORE normalization (to match training data format)
    ...
    def __call__(self, ob, goal=None, batched_ob=False):
        ...
        Args:
            ...
            batched_ob (bool): whether the input is already batched
        """
        ob = self._prepare_observation(ob, batched_ob=batched_ob) # (batch, To, *dim)
        if goal is not None:
